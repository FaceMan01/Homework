...
void Sort(vector<tuple<int, int, int>> &mas, int first, int last)
{
    int middle;
...
public:
    ...
    AdjacencyMatrix(vector<vector<int>> matrix, tuple<bool, bool, int> info) {
        adj_matrix = matrix;
...
public:
    ...
    AdjacencyList(vector<set<pair<int, int>>> list, tuple<bool, bool, int> info) {
        adj_list = list;
        is_weighted = get<0>(info);
        is_oriented = get<1>(info);
        vertex_num = get<2>(info);
...
class ListOfEdges: public GraphRepresentation {
private:
    int num_of_edges;
    vector <tuple<int, int, int>> list_of_edges;
public:
    ...
    ListOfEdges(vector <tuple<int, int, int>> list, tuple<bool, bool, int> info) {
        list_of_edges = list;
        is_weighted = get<0>(info);
        is_oriented = get<1>(info);
        vertex_num = get<2>(info);
    ...
    vector <tuple<int, int, int>> getSpaingTreeKruscal() {
        ...
        Sort(list_of_edges, 0, list_of_edges.size() - 1);
...
class Graph {
private:
    GraphRepresentation* repres = nullptr;
public:
    Graph() {
    ...
    void readGraph(string fileName) {
        ...
        switch (repr)
        {
        case 'C':   repres = new AdjacencyMatrix(); break;
        case 'L':   repres = new AdjacencyList(); break;
        case 'E':   repres = new ListOfEdges(); break;
        }
    ...
    void transformToAdjMatrix() {
        repres = new AdjacencyMatrix(repres->transformToAdjMatrix(), repres->GetInfo());
    }
 
    void transformToAdjList() {
        repres = new AdjacencyList(repres->transformToAdjList(), repres->GetInfo());
    }
 
    void transformToListOfEdges() {
        repres = new ListOfEdges(repres->transformToListOfEdges(), repres->GetInfo());
    }
    ...
    Graph getSpaingTreePrima() {
        ...
        vector<tuple<int, int, int>> minimalSpanningTree = reinterpret_cast<AdjacencyList*>(repres)->getSpaingTreePrima();
        Graph* spanningTree = new Graph();
        spanningTree->repres = new ListOfEdges(minimalSpanningTree, repres->GetInfo());
        return *spanningTree;
    }
 
    Graph getSpaingTreeKruscal() {
        this->transformToListOfEdges();
        vector <tuple<int, int, int>> minimalSpanningTree = reinterpret_cast<ListOfEdges*>(repres)->getSpaingTreeKruscal();
        Graph* spaing_tree = new Graph();
        spaing_tree->repres = new ListOfEdges(minimalSpanningTree, repres->GetInfo());
        return *spaing_tree;
    }

    Graph getSpaingTreeBoruvka() {
        this->transformToListOfEdges();
        vector <tuple<int, int, int>> minimalSpanningTree = reinterpret_cast<ListOfEdges*>(repres)->getSpaingTreeBoruvka();
        Graph* spaing_tree = new Graph();
        spaing_tree->repres = new ListOfEdges(minimalSpanningTree, repres->GetInfo());
        return *spaing_tree;
    }
